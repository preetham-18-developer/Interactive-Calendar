...
export const Radar = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-40 h-[1px] w-full bg-gradient-to-r from-transparent via-[var(--color-accent)] to-transparent" />
      </div>
      {/* Concentric circles */}
      {circles.map((_, idx) => (
        <Circle
  ...
  return (
    ...
    >
      ...
      <div className="hidden rounded-md px-2 py-1 md:block">
        <div className="text-center text-[10px] font-bold text-slate-400 max-w-[80px] truncate">
          {text || "Event"}
        </div>
      </div>
    </motion.div>
